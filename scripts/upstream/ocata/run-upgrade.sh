...
function main {
    ...
    pushd ${MAIN_PATH}/playbooks
        ...
        # Skip setup-openstack task as pike should handle the db upgrades
        #RUN_TASKS+=("setup-openstack.yml")
        # Run the tasks in order
        for item in ${!RUN_TASKS[@]}; do
          run_lock $item "${RUN_TASKS[$item]}"
        done
        echo "setup-openstack.yml is skipped in this script. If you want to"
        echo "upgrade to only Ocata at this time, please run the"
        echo "setup-openstack.yml at this point."
    popd
}
